...
it settled by whether the expert tensor is factorised at all. Tensor-Train
shares a basis as thoroughly as Tucker does, and keeps it as clean
...
Tensor-Train at $1.1$--$1.2$ from $N = 256$ upward with purity above $0.94$.
...
generalisation, but only loosely: over the $78$ generalising mixture runs the
rank correlation between effective frequency count and grokking step is
$\rho = 0.54$ (Figure~\ref{fig:freqgrok}). These runs share seeds and their
...
are in bold. Every architecture decays; none recovers.}
...
$0.62$ against a noise floor of $0.082$. By $N = 4096$ every router has fallen to between $0.048$ and $0.093$: at or
below the noise floor for five of the six architectures, and within $14\%$ of it
for the sixth. The decay holds across all six architectures --- the ones that generalise faster with scale and the ones
...
computation sits entirely in the shared factors, and the routing weights carry
less frequency structure than random numbers. Figure~\ref{fig:purity} compares
...
as steeply as on the single task. Adding a second subtask narrows both windows.
...
No factor on the first side of the router is ever flagged. With
$d_{\text{in}} = 228$ the router's split places the whole of $\mathrm{onehot}(a)$
on the first side and the task token on the second, so the first side cannot
...
Four ablations at $N = 256$ and $N = 4096$, seed $0$, isolate mechanisms that
...
\begin{table}[ht]
\centering
\caption{Ablations, seed 0. ``fail'' gives the final test accuracy in
parentheses. The baseline configuration is a natural input split,
$\lambda = 10^{-3}$, router keys decayed, $80\%$ training fraction.}
\label{tab:ablations}
\footnotesize
\begin{tabular}{llrrr}
\toprule
...
& MONET-HD, $N{=}256$     & $7600$ & $6600$ & \\
& MONET-HD, $N{=}4096$    & $17600$ & fail $(0.090)$ & \\
...
inputs the router keys form a table of $4nP$ parameters addressed directly by
the operand value --- $7232$ of them at $N = 256$ and $28\,928$ at $N = 4096$.
Exempting them from weight decay, as is conventional for router parameters,
leaves that table unregularised. At the $80\%$ training fraction used
throughout, the cost appears only where the table is largest: MONET-HD at
$N = 4096$ stops generalising entirely, while at $N = 256$ the difference
($7600$ against $6600$) sits well inside that configuration's seed spread of
$\pm 1986$. For Tucker the ablation makes no difference at either expert count.
...
for Tucker it is an accelerant.
...
$800$ as $\lambda$ rises --- a mild cost. For MONET-HD the three values give
$6500$, $7600$ and $3000$, which is not monotone and sits inside the seed
spread for that configuration. Since these terms were calibrated for language
...
rank does not help at any expert count. It hurts everywhere, and beyond rank
$16$ it opens a failure region that widens as the rank grows: at rank $64$ and
...
Table~\ref{tab:scaling} and the parameter counts of Table~\ref{tab:params}.}
\label{tab:comp}
\footnotesize
\begin{tabular}{lccc}
\toprule
...
\midrule
dense   & $600$ \quad ($288$K) & $500$ \quad ($1090$K) & $533$ \quad ($1121$K) \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{A threshold, then nothing.} Table~\ref{tab:comp} shows a sharp
transition rather than a gradual trade-off. At ranks $1$ and $2$ the layer
cannot fit the training set, let alone generalise. At rank $4$ it fits but
...
$N = 256$ to $N = 4096$ falls from $3.89\times$ to $1.81\times$. A coupling of
rank $32$ out of a possible $1024$, or of rank $8$ out of $1024$ at a cost of
one generalisation step in five, is all the task uses.
