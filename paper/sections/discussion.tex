...
keys form a table of $4nP$ parameters addressed directly by the operand, and it
...
generalises sooner or later with scale, the spectral purity of the router keys
falls as experts are added, reaching the white-noise floor by $N = 4096$. In
...
which leaves the Tensor-Train trend without a mechanism. The first in particular rests on an
architectural detail of our own configuration and has not been tested by
varying the ranks.
...
withheld. The ablations use one seed, so the differences reported there are not
protected against the seed variance that Section~\ref{sec:res-stability}
documents for the product-key layers in particular.
...
makes the hierarchical format a way of removing it by construction rather
than by a rank constraint imposed after the fact.
...
Table~\ref{tab:params} while CP's and Tensor-Train's do not. The hierarchical
Tucker format \citep{grasedyck2010hierarchical} removes that growth: instead of
one dense core, the modes are arranged on a binary tree with a small transfer
tensor at each internal node, giving parameter count linear rather than
exponential in the order while retaining the pairwise mode interactions that CP
lacks.
