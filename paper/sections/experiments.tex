...
transition occurs at step $2000$. Weight decay $3.0$ keeps test accuracy below the threshold at every fraction, reaching $0.775$ at best.

\begin{table}[t]
\centering
\caption{Dense MLP baseline, seed $0$: grokking step and final test accuracy
across training regimes. A grokking step of ``---'' means the $95\%$ threshold
was never reached within $30\,000$ steps. The $80\%$ row is a single seed and
gives $2000$, against the three-seed mean $1667 \pm 58$ of
...
weight decay. Under one-hot inputs the keys form a lookup table of $8 n P$
...
\subsection{What the single task cannot measure}
