...
(rank $128$), Tucker (rank $32$) and Tensor-Train (rank $16$). Expert dimension is $m = 8$ throughout. For the Tucker layer the input and
output modes use rank $32$ and the two expert modes rank $\min(32, n)$; CP uses
a single shared rank of $128$ and Tensor-Train a uniform bond rank of $16$.
These ranks are not matched across architectures, and
Section~\ref{sec:limitations} returns to what that leaves open.
...
top-$k$; at $N \le 4096$ an exact top-$k$ is inexpensive, so we use it, and retain BatchNorm as an option, which is disabled in every run reported here. Second, the auxiliary losses below are computed
...
initialisation at fixed data, not variation due to resampling. We use three seeds for the main grid and for the router-key decay ablation;
the remaining ablations and sweeps use seed $0$ and are marked as such.
