...
failure for independent experts beyond 64. Second, factorisations with a shared
basis compress the frequency content of the learned representation by more than
an order of magnitude relative to a dense layer, and the product-key layers lose
...
keys falls to the white-noise floor in every mixture architecture we test, to
0.048--0.093 against a floor of 0.082, while a Tucker layer's shared basis
...
На этом тестбеде мы обучаем по единому протоколу семь типов слоёв: плотный
...
CP-слоёв, растёт у TT- и продуктовых слоёв и переходит в полный отказ у
независимых экспертов. Во-вторых, разложения с общим базисом сжимают частотное
содержание выученного представления более чем на порядок по сравнению с плотным
слоем, а продуктовые слои теряют это сжатие по мере роста. 

В-третьих, масштабирование лишает роутер алгоритмической структуры, но не
всякой: при 4096 экспертах спектральная чистота ключей падает до уровня белого
шума во всех рассмотренных архитектурах --- до $0{,}048$--$0{,}093$ при пороге
$0{,}082$, --- тогда как общий базис Tucker-слоя остаётся чистым. При этом на
